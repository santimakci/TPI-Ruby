...
#     yield
#   rescue TimeoutError
#     raise TimeoutError, 'Connection to the server was timed out'
#   rescue Errno::ECONNRESET, Errno::EPIPE, Errno::ECONNABORTED, Errno::EBADF, Errno::EINVAL
#     raise Redis::ConnectionError, 'Connection to the server was lost'
#   end
# 
#   protected
...
#     rescue TimeoutError
#       raise Redis::CannotConnectError, 'Timed out connecting to ' + location
#     rescue Errno::ECONNREFUSED
#       raise Redis::CannotConnectError, 'Error connecting to '  + location
#     end
# 
...
#         if connected?
#           if Process.pid != @pid
#             raise Redis::InheritedError, 'You need to reconnect to after forking'
#           end
#         else
#           connect
#         end
...
class Redis
  ...
  protected
    ...
    def establish_connection
      ...
      raise CannotConnectError, 'Timed out connecting to ' + location
    rescue Errno::ECONNREFUSED
      raise CannotConnectError, 'Error connecting to '  + location
    end
    ...
    def ensure_connected
      ...
      begin
        if connected?
          if Process.pid != @pid
            raise InheritedError, 'You need to reconnect after forking'
          end
        else
          connect
        end
