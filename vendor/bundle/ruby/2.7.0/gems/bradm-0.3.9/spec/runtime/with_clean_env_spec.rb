...
describe "Bundler.with_env helpers" do
  ...
  around do |example|
    env = Bundler::BRADM_ORIGINAL_ENV.dup
    Bundler::BRADM_ORIGINAL_ENV['BUNDLE_PATH'] = "./Gemfile"
    example.run
    Bundler::BRADM_ORIGINAL_ENV.replace env
  end
  ...
  describe "Bundler.with_clean_env" do
    ...
    it "should not pass RUBYOPT changes" do
      ...
      Bundler::BRADM_ORIGINAL_ENV['RUBYOPT'] = " -I#{lib_path} -rbundler/setup"
      ...
      expect(Bundler::BRADM_ORIGINAL_ENV['RUBYOPT']).to eq(" -I#{lib_path} -rbundler/setup")
    end

    it "should not change BRADM_ORIGINAL_ENV" do
      expect(Bundler::BRADM_ORIGINAL_ENV['BUNDLE_PATH']).to eq('./Gemfile')
    end

  end

  describe "Bundler.with_bradm_original_env" do

    it_should_behave_like "Bundler.with_*_env"

    it "should pass bundler environment variables set before Bundler was run" do
      Bundler.with_bradm_original_env do
