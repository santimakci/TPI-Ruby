...
describe "bundle clean" do
  ...
  it "removes unused gems that are different" do
    ...
    bundle "install --path ../../.gex/deploy --no-clean"
  ...
  it "does not remove git sources that are in without groups" do
    ...
    G
    bundle "install --path ../../.gex/deploy --without test"
  ...
  it "does not blow up when using without groups" do
    ...
    bundle "install --path ../../.gex/deploy --without development"
  ...
  it "removes .gex/.gemspec file even if there's no corresponding gem dir" do
    gemfile <<-G
      source "file://#{gem_repo1}"
  ...
  it "--clean should override the bundle setting on install" do
    gemfile <<-G
      ...
      gem "rack"
    G
    bundle "install --path ../../.gex/deploy --clean"
  ...
  it "doesn't blow up on path gems without a .gempsec" do
    relative_path = ".gex/private_gems/bar-1.0"
