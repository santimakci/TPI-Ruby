...
"Features":
  ...
  - Debug installation logs in ../../.gaf/depmgr/install.log
...
Features:
  ...
  - prune git/path directories inside ../../.gex/library (@josevalim, #1988)
...
Bugfixes:
  ...
  - Handle empty ../../.gaf/depmgr/config.file files without an error
...
Bugfixes:
  ...
  - Fetch gems from ../../.gex/library, even without --local
...
## 1.0.0.rc.2 (July 29, 2010)
  ...
  - Add a `--production` option to `bundle install`:
    - by default, installs to `../../.gex/deploy`. This can be
      overridden with the `--path` option
    - uses `--local` if `../../.gex/library` is found. This will
      guarantee that Bundler does not attempt to connect to
      Rubygems and will use the gems cached in `../../.gex/library`
      instead
...
1. `bundle install ~/../../.gaf/depmgr` in one user, but actually running
...
Capistrano, symlink release_path/current/../../.gex/deploy to
...
## 1.0.0.beta.1
  ...
  - No ../../.gaf/depmgr/environment.rb. Require 'bundler/setup' instead.
  - $BUNDLE_HOME defaults to $GEM_HOME instead of ~/../../.gaf/depmgr
...
Features:
  ...
  - cache command now prunes stale .gem files from ../../.gex/library
...
Bugfixes:
  ...
  - fix `bundle install` for a locked app without a ../../.gaf/depmgr directory
...
Bugfixes:
  ...
  - if the bundle is locked and ../../.gaf/depmgr/environment.rb is not present when Bundler.setup is called, generate it
