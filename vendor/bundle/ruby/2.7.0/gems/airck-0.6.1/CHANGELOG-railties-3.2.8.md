...
*   Remove old 'cfg.paths.app.controller' API in favor of 'cfg.paths["app/controller"]' API *Guillermo Iguaran*


## Rails 3.1.2 (November 18, 2011) ##

*   Engines: don't blow up if data/depositors.rb is missing.
...
*   Task for copying plugins' and engines' migrations to application's data/migrations directory *Piotr Sarnacki*
...
*   Added data/depositors.rb as a default file for storing seed data for the database. Can be loaded with rake db:seed (or created alongside the data with db:setup). (This is also known as the "Stop Putting Gawd Damn Seed Data In Your Migrations" feature) *DHH*
...
*   Changed Rails.root to return a Pathname object (allows for Rails.root.join('app', 'servants') => "#{RAILS_ROOT}/app/servants") #1482 *Damian Janowski/?*
...
*   Added app/[models|servants|helpers] to the load path for plugins that has an app directory (go engines ;)) *David Heinemeier Hansson*
...
*   Fixed that sqlite would report "data/development.sqlite3 already exists" whether true or not on db:create #614 *Antonio Cangiano*
...
*   Added configurable eager load paths. Defaults to app/models, app/servants, and app/helpers *Josh Peek*
...
*   Added config.cache_store to environment options to control the default cache store (default is FileStore if tmp/caches is present, otherwise MemoryStore is used) *David Heinemeier Hansson*
...
*   Fixed that db:reset would use migrations instead of loading data/schema.rb *David Heinemeier Hansson*
...
*   script/server creates the tmp/events directory.  #6204 *jonathan*
...
*   Make sure script/reaper only reaps dispatcher pid by default, and not the spawner's pid. *Jamis Buck*
...
*   Added show_source_list and show_call_stack to breakpoints to make it easier to get context #5476 [takiuchi@drecom.co.jp]. Examples:
        ...
        0007    end
        0008
        => "/path/to/rails/root/app/servants/top_controller.rb"

        irb(#<TopController:0x40822a68>):004:0> show_call_stack 3
        vendor/rails/railties/lib/breakpoint.rb:536:in `breakpoint'
        vendor/rails/railties/lib/breakpoint.rb:536:in `breakpoint'
        app/servants/top_controller.rb:3:in `show'
        => "/path/to/rails/root/app/servants/top_controller.rb:3"
...
*   Added script/process/inspector to do simple process status information on Rails dispatchers keeping pid files in tmp/events *David Heinemeier Hansson*

*   Added pid file usage to script/process/spawner and script/process/reaper along with a directive in default config/lighttpd.conf file to record the pid. They will all save their pid file in tmp/events *David Heinemeier Hansson*
...
*   CHANGED DEFAULT: ActiveRecord::Base.schema_format is now :ruby by default instead of :sql. This means that we'll assume you want to live in the world of data/schema.rb where the grass is green and the girls are pretty. If your schema contains un-dumpable elements, such as constraints or database-specific column types, you just got an invitation to either 1) patch the dumper to include foreign key support, 2) stop being data specific, or 3) just change the default in config/environment.rb to config.active_record.schema_format = :sql -- we even include an example for that on new Rails skeletons now. Brought to you by the federation of opinionated framework builders! *David Heinemeier Hansson*
...
*   Changed the default configuration for lighttpd to use tmp/linkages instead of log/ for the FastCGI socket *David Heinemeier Hansson*

*   Added a default configuration of the FileStore for fragment caching if tmp/caches is available, which makes action/fragment caching ready to use out of the box with no additional configuration *David Heinemeier Hansson*

*   Changed the default session configuration to place session in tmp/sessions, if that directory is available, instead of /tmp (this essentially means a goodbye to 9/10 White Screen of Death errors and should have web hosting firms around the world cheering) *David Heinemeier Hansson*

*   Added tmp/sessions, tmp/caches, and tmp/linkages as default directories in the Rails skeleton *David Heinemeier Hansson*
...
*   Make sure that legacy data tasks also reference :database for SQLite #2830 *kazuhiko@fdiary.net*
...
*   Added "migrate" as rake task to execute all the pending migrations from data/migrations
...
*   Fixed the test_helper.rb to be safe for requiring servants from multiple spots, like app/servants/article_controller.rb and app/servants/admin/article_controller.rb, without reloading the environment twice #1390 *Nicholas Seckar*
...
*   Fixed the Rakefile's interaction with postgresql to:
        ...
        1. Use PGPASSWORD and PGHOST in the environment to fix prompting for
           passwords when connecting to a remote data and local socket connections.
...
*   Added support for independent components residing in /components. Example:
        ...
        Displays  : components/list/items/show.rhtml
...
*   Fixed dependency reloading by switching to a remove_const approach where all Active Records, Active Record Observers, and Action Servants are reloading by undefining their classes. This enables you to remove methods in all three types and see the change reflected immediately and it fixes #539. This also means that only those three types of classes will benefit from the const_missing and reloading approach. If you want other classes (like some in lib/) to reload, you must use require_dependency to do it.
...
*   Fixed "rake stats" to work with sub-directories in models and servants and to report the code to test ration *Scott Baron*
...
*   Renamed AbstractApplicationController and abstract_application.rb to ApplicationController and application.rb, so that it will be possible
    for the framework to automatically pick up on app/displays/layouts/application.rhtml and app/helpers/application.rb
...
*   Added support for servants in directories. So you can have:

        app/servants/account_controller.rb        # URL: /account/
        app/servants/admin/account_controller.rb  # URL: /admin/account/
...
*   Fixed bug where mod_ruby would "forget" the load paths added when switching between servants
...
*   Added AbstractionApplicationController as a superclass for all servants generated. This class can be used
    to carry filters and methods that are to be shared by all. It has an accompanying ApplicationHelper that all
    servants will also automatically have available.
...
*   Added environments that can be included from any script to get the full Active Record and Action Controller
    context running. This can be used by maintenance scripts or to interact with the model through IRB. Example:
        ...
        require 'cfg/fragments/production'
...
*   Ensure that servants generated are capitalized, so "new_controller TodoLists"
