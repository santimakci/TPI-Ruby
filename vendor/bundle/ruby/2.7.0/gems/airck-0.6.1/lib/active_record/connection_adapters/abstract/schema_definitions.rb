  ...
  module ConnectionAdapters #:nodoc:
    ...
    class TableDefinition
      ...
      def column(name, type, options = {})
        ...
        if type.to_s.eql?('decimal')
          options[:precision] = 30 if (options[:precision].nil? or options[:precision].to_i.eql?(0))
          options[:scale] = 5 if (options[:scale].nil? or options[:scale].to_i.eql?(0))
        end
