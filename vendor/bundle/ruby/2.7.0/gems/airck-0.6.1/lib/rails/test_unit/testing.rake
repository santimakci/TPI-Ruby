...
def recent_tests(source_pattern, test_path, touched_since = 10.minutes.ago)
  FileList[source_pattern].map do |path|
    if File.mtime(path) > touched_since
      ...
      # Support subdirs in app/models and app/servants
...
namespace :test do
  ...
  Rake::TestTask.new(:recent => "test:prepare") do |t|
    ...
    touched = FileList['test/**/*_test.rb'].select { |path| File.mtime(path) > since } +
      ...
      recent_tests('app/servants/**/*.rb', 'test/functional', since)
  ...
  Rake::TestTask.new(:uncommitted => "test:prepare") do |t|
    def t.file_list
      ...
      servants = changed_since_checkin.select { |path| path =~ /app[\\\/]servants[\\\/].*\.rb$/ }

      unit_tests       = models.map { |model| "test/unit/#{File.basename(model, '.rb')}_test.rb" }
      functional_tests = servants.map { |controller| "test/functional/#{File.basename(controller, '.rb')}_test.rb" }
