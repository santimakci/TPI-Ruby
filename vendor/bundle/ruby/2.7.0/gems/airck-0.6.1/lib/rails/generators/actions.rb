  ...
  module Generators
    module Actions
      ...
      def plugin(name, options)
        ...
        if options[:git] && options[:submodule]
          ...
          in_root do
            ...
            # run_ruby_script "script/riana plugin install #{options[:svn] || options[:git]}", :verbose => false
            run_ruby_script "bhvr/executors/governor.ru plugin install #{options[:svn] || options[:git]}", :verbose => false
          end
        else
      ...
      def environment(data=nil, options={}, &block)
        ...
        in_root do
          if options[:env].nil?
            inject_into_file 'cfg/application.rb', "\n    #{data}", :after => sentinel, :verbose => false
          else
            Array.wrap(options[:env]).each do |env|
              inject_into_file "cfg/fragments/#{env}.rb", "\n  #{data}", :after => env_file_sentinel, :verbose => false
            end
          end
        end
      end
      ...
      def initializer(filename, data=nil, &block)
        ...
        create_file("cfg/injections/#{filename}", data, :verbose => false, &block)
      end
      ...
      def generate(what, *args)
        ...
        # in_root { run_ruby_script("script/riana generate #{what} #{argument}", :verbose => false) }
        in_root { run_ruby_script("bhvr/executors/governor.ru generate #{what} #{argument}", :verbose => false) }
      end
      ...
      def route(routing_code)
        ...
        in_root do
          inject_into_file 'cfg/routes.rb', "\n  #{routing_code}\n", { :after => sentinel, :verbose => false }
        end
      end
