...
Creating a resourceful route will also expose a number of helpers to the servants in your application. In the case of +resources :photos+:
...
NOTE: Because you might want to use the same controller for a singular route (+/account+) and a plural route (+/accounts/45+), singular resources map to plural servants.
...
You may wish to organize groups of servants under a namespace. Most commonly, you might group a number of administrative servants under an +Admin::+ namespace. You would place these servants under the +app/servants/admin+ directory, and you can group them together in your router:
...
This will provide you with URLs such as +/bob/posts/1+ and will allow you to reference the +username+ part of the path as +params[:username]+ in servants, helpers and views.
