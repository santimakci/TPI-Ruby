...
which migrations should be run. It will also update your +data/schema.rb+ file to
...
Migrations are stored as files in the +data/migrations+ directory, one for each
...
data/migrations/YYYYMMDDHHMMSS_add_details_to_products.rb file.
...
will update your data/schema.rb file to match the structure of your database.
...
<ruby>
# data/migrations/20100513121110_add_flag_to_product.rb
...
<ruby>
# data/migrations/20100515121110_add_fuzz_to_product.rb
...
database schema. That role falls to either +data/schema.rb+ or an SQL file which
...
database is dumped (either to +data/schema.rb+ or +data/structures.sql+) and then
...
If +:ruby+ is selected then the schema is stored in +data/schema.rb+. If you look
...
There is however a trade-off: +data/schema.rb+ cannot express database specific
...
into +data/structures.sql+. For example, for the PostgreSQL RDBMS, the
...
foreign keys in +data/schema.rb+).
