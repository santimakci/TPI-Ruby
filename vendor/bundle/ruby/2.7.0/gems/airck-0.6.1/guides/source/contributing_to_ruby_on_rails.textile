...
The gem +sqlite3-ruby+ does not belong to the "data" group indeed, if you followed the instructions above you're ready. This is how you run the Active Record test suite only for SQLite3:
...
We need first to delete +.bundle/config+ because Bundler remembers in that file that we didn't want to install the "data" group (alternatively you can edit the file).
