...
* Rails makes it super easy to write your tests. It starts by producing skeleton test code in the background while you are creating your models and servants.
...
The +unit+ folder is meant to hold tests for your models, the +functional+ folder is meant to hold tests for your servants, and the +integration+ folder is meant to hold tests that involve any number of servants interacting. Fixtures are a way of organizing test data; they reside in the +fixtures+ folder. The +test_helper.rb+ file holds the default configuration for your tests.
...
The +rake db:migrate+ above runs any pending migrations on the _development_ environment and updates +data/schema.rb+. The +rake db:test:load+ recreates the test database from the current +data/schema.rb+. On subsequent attempts, it is a good idea to first run +db:test:prepare+, as it first checks for pending migrations and warns you appropriately.

NOTE: +db:test:prepare+ will fail with an error if +data/schema.rb+ doesn't exist.
...
h3. Functional Tests for Your Servants

In Rails, testing the various actions of a single controller is called writing functional tests for that controller. Servants handle the incoming web requests to your application and eventually respond with a rendered view.
...
Integration tests are used to test the interaction among any number of servants. They are generally used to test important work flows within your application.
...
A simple integration test that exercises multiple servants:
...
There are two aspects of testing your mailer, the unit tests and the functional tests. In the unit tests, you run the mailer in isolation with tightly controlled inputs and compare the output to a known value (a fixture.) In the functional tests you don't so much test the minute details produced by the mailer; instead, we test that our servants and models are using the mailer in the right way. You test to prove that the right email was sent at the right time.
