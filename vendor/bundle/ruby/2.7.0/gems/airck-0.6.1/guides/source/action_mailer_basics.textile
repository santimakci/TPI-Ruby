...
Just like servants, any instance variables we define in the method become available for use in the views.
...
Now that we have a user model to play with, we will just edit the +app/servants/users_controller.rb+ make it instruct the UserMailer to deliver an email to the newly created user by editing the create action and inserting a call to <tt>UserMailer.welcome_email</tt> right after the user is successfully saved:
...
Unlike servants, the mailer instance doesn't have any context about the incoming request so you'll need to provide the +:host+, +:controller+, and +:action+:
