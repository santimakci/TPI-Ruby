...
You've heard that Rails promotes "convention over configuration". Default rendering is an excellent example of this. By default, servants in Rails automatically render views with names that correspond to valid routes. For example, if you have this code in your +BooksController+ class:
...
WARNING: There is seldom any good reason to use this option. Mixing ERB into your servants defeats the MVC orientation of Rails and will make it harder for other developers to follow the logic of your project. Use a separate erb view instead.
...
h6. Specifying Layouts for Servants

You can override the default layout conventions in your servants by using the +layout+ declaration. For example:
